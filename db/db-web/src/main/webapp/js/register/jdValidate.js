	...
	isNull : {
		...
		run : function(option, str) {
			...
			if (str == "") {
				...
				option.isNullEle.removeClass().addClass(
						validateSettings.error.style).html(str);
			}
		}
	},
	error : {
...
var validateFunction = {
	...
	pwd : function(option) {
		...
		if ((validateRules.isNull(regName) == false) && (regName != '')
				&& regName == str1) {
			$("#pwdstrength").hide();
			validateSettings.error.run(option,
					"<span>您的密码与账户信息太重合，有被盗风险，请换一个密码</span>");
			return;
		}
...
function checkPin(option) {
	...
	if (!namestate || nameold != pin) {
		if (nameold != pin) {
			...
			$.ajax({
				url : "http://sso.db.com/user/check/" + escape(pin) + "/1?r="
						+ Math.random(),
				dataType : "jsonp",
			...
			if (checkpin == 2) {
				...
				validateSettings.error.run(option, "<span>"
						+ option.prompts.error.beUsed.replace("{1}",
								option.value) + "</span>");
				if (morePinOld != null && morePinOld.length > 0) {
					$("#morePinDiv").removeClass()
							.addClass("intelligent-error");
				}
			}
			namestate = false;
		}
...
function sendMobileCode() {
	...
	if (!validateRules.isMobile(mobile)) {
		$("#phone_error").removeClass().addClass("error").html(
				"手机号码格式有误，请输入正确的手机号");
		$("#phone_error").show();
		return;
	}
	...
	// 检测手机号码是否存在
	$.ajax({
		url : "http://sso.db.com/user/check/" + mobile + "/2?r="
				+ Math.random(),
		dataType : "jsonp",
...
function sendmCode() {
	...
	jQuery.ajax({
		type : "get",
		url : "../notifyuser/mobileCode?state=" + state + "&mobile="
				+ $("#phone").val() + "&r=" + Math.random(),
		success : function(result) {
			if (result) {
				var obj = eval(result);
				if (obj.rs == 1 || obj.remain) {
					...
					if (obj.remain) {
						...
						if (state == "unbind") {
							$("#mobileCode_error").removeClass()
									.addClass("cue").empty().html(
											"校验码已发送,注册成功后手机号将与原帐号解绑");
							$("#mobileCode_error").show();
						} else {
							$("#mobileCode_error").removeClass().empty().html(
									"验证码已发送，请查收短信。");
							$("#mobileCode_error").show();
						}
					}
					...
					$("#sendMobileCode").removeClass().addClass("btn btn-15")
				...
				success : function(result) {
					if (result) {
						...
						if (obj.rs == 1 || obj.remain) {
							...
							if (obj.remain) {
								$("#mobileCodeSucMessage1").empty().html(
										obj.remain);
							} else {
								if (state == "unbind") {
							...
							$("#sendMobileCode1").removeClass().addClass(
									"btn btn-15").attr("disabled", "disabled");
							$("#mobileCode1").removeAttr("disabled");
						}
						if (obj.rs == -1) {
							$("#mobileCode1_error").removeClass().addClass(
									"error").html("网络繁忙，请稍后重新获取验证码");
							$("#sendMobileCode1").removeClass().addClass("btn")
									.removeAttr("disabled");
						}
						if (obj.info) {
							if (obj.info == "该手机号已被使用，请更换号码") {
...
function sendMobileCode1() {
	...
	if (!validateRules.isMobile(mobile)) {
		$("#phone1_error").removeClass().addClass("error").html(
				"手机号码格式有误，请输入正确的手机号");
		$("#phone1_error").show();
		$("#phone1_succeed").removeClass().addClass("blank error-ico");
		return;
...
function sendRegMail() {
	...
	if (!format) {
		...
		if (!format2) {
			...
			// }
			// oldEmail = email;
			$.getJSON("../validateuser/isEmailEngaged?email=" + escape(email)
...
function mobileReg() {
	...
	if (mobileFlag) {
		...
		$
				.ajax({
					type : "POST",
					url : "../register/sendRegEmail?r=" + Math.random() + "&"
						...
						if (emailResult == 0) {
							...
							jQuery
									.jdThickBox({
										...
										width : 510,
										height : 280,
										source : '<div class="thickbox-tip fz14">'
												+ '<div class="icon-box">'
												...
												+ '</div>'
												+ '<div class="msg-txt">'
												+ '系统已向您的邮箱&nbsp;<strong class="ftx-01">'
												+ $("#emailMg").val()
												+ '</strong>&nbsp;发送了一封验证邮件，请您登录邮箱，点击邮件中的链接完成邮箱验证。如果超过2分钟未收到邮件，您可以<a href="#none" onclick="reSendEmail(\''
												+ $("#emailMg").val()
												+ '\',\''
												+ key
												+ '\');" class="ftx-05">重新发送</a>'
												+ '</div>'
												+ '<div class="mt10 ftx-01"> <span id="reSendEmailSuccess"></span></div>'
												+ '<div class="mt20">'
												+ '<a href="#" id="emailLogin" class="btn-red">登录邮箱</a>'
												// +'<a href="#none"
												// onclick="changeEmail();"
...
function countDown() {
	...
	if (delayTime == 1) {
		...
		$("#mobileCode_error").addClass("hide");
		$("#sendMobileCode").removeClass().addClass("btn").removeAttr(
				"disabled");
		delayFlag = true;
	} else {
		delayFlag = false;
...
function countDown1() {
	...
	if (delayTime1 == 1) {
		...
		$("#mobileCode1_error").removeClass().empty();
		$("#mobileCode1_error").hide();
		$("#sendMobileCode1").removeClass().addClass("btn").removeAttr(
				"disabled");
		delayFlag1 = true;
	} else {
		delayFlag1 = false;
...
function phoneFocus() {
	...
	$("#phone_error").removeClass().addClass("focus").html(
			"完成验证后，您可以用该手机号登录和找回密码");
	$("#phone_error").show();
...
function phone1Focus() {
	...
	$("#phone1_error").removeClass().addClass("focus").html(
			"完成验证后，您可以用该手机号登录和找回密码");
	$("#phone1_error").show();
	$('#phone1_succeed').removeClass('error-ico');
}
...
function phoneOtherBlur() {
	...
	// 检测手机号码是否存在
	$.getJSON("../validateuser/isMobileEngaged?mobile=" + mobile + "&r="
...
function mobileEngagedStyle() {
	...
	// $("#phone_succeed").removeClass().addClass("blank cue-ico");
	// $("#rebind").remove();
	// $('#dphone').after('<div class="item" id="rebind"> <span
	// class="label">&nbsp;</span><div class="fl item-ifo item-ifo-extra"> <a
	// href="javascript:;" onclick="unbind();"
	// class="btn-comm"><span>尝试解绑手机号</span></a> </div> </div>');
	// $("#mobileCodeDiv").hide();
	mobileFlags = false;
}
...
function mobileEngagedStyle1() {
	...
	$('#dphone1')
			.after(
					'<div class="item"  id="rebind1"><span class="label">&nbsp;</span><div class="fl item-ifo"><a href="javascript:void(0);" onclick="unbind1();"  class="btn btn-comm"><span>尝试解绑手机号</span></a></div></div>');
	$("#mobileCodeDiv1").hide();
	mobileFlag = false;
	return;
}
						...
						jQuery
								.jdThickBox({
									...
									source : "<div class=\" regist-2013\">"
											...
											"    <p>"
											+ "        <strong>3.6</strong><strong>用户同意，京东有权使用用户的注册信息、用户名、密码等信息，登录进入用户的注册账户，进行证据保全，包括但不限于公证、见证等。</strong></p>"
											+ "    <h5> 第4条 用户依法言行义务</h5>"
											+
											...
											+ "        以商品出库为标志），方视为您与销售方之间就实际直接向您发出的商品建立了合同关系；如果您在一份订单里订购了多种商品并且销售方只给您发出了部分商品时，您与销售方之间仅就实际直接向您发出的商品建立了合同关系；只有在销售方实际直接向您发出了订单中订购的其他商品时，您和销售方之间就订单中该其他已实际直接向您发出的商品才成立合同关系。您可以随时登录您在本站注册的账户，查询您的订单状态。</strong>"
											+ "    </p>"
											+
